...
prompt_virtualenv() {
  ...
  if [[ -n "$virtualenv_path" && "$VIRTUAL_ENV_DISABLE_PROMPT" != true ]]; then
    "$1_prompt_segment" "$DEFAULT_COLOR" 'PYTHON_ICON'
  fi
}
...
zplug "plugins/pip3",              from:oh-my-zsh, if:"(( $+commands[pip3] ))"
...
if zplug check "bhilburn/powerlevel9k"; then
    ...
    POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(command_execution_time  background_jobs status virtualenv time ssh)
...
# vim: ft=zsh
# Message to display on Terminal by Shin added 06/02/2019
python Shin.py
